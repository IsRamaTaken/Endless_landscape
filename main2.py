...
while running:
    ...
    if compteur_de_frame - tchangement >= tempsDureeMouvement and posXatteint and posYatteint:
        ...
        for i in range(len(vzoom)):
            ...
            sizeWx = int(size_x / Z)
            sizeWy = int(size_x / Z)
            print('pos', posY - sizeWy // 2 + 1 - size_y % 2 >= 0, posY + sizeWy // 2 < limite_up_y - 1,
                  posX - sizeWx // 2 + 1 - size_x % 2 >= 0, posX + sizeWx // 2 <= limite_up_x - 1, Z)
            if posY - sizeWy // 2 + 1 - size_y % 2 >= 0 and posY + sizeWy // 2 < limite_up_y - 1 and posX - sizeWx // 2 + 1 - size_x % 2 >= 0 and posX + sizeWx // 2 <= limite_up_x - 1:
                VzoomPossible.append(vzoom[i])

        Zt += choice(VzoomPossible) / framerate
        ...
        if not type_zoom:
            ...
            probaZoom, probaDezoom, sens_deplacement_x, direction_deplacement_x, sens_deplacement_y, direction_deplacement_y, temps_debut_zoom, directionZoom, zinit, zf, indice_zoom, zoom_en_cours_Auto, temps_changement_zoom, Zt \
                = zoom_automatique(probaZoom, probaDezoom, sens_deplacement_x, direction_deplacement_x,
                                   sens_deplacement_y, direction_deplacement_y, Zt, vitesse_actuelle_x,
                                   vitesse_actuelle_y, vzoom, posX, posY, size_x, size_y, limite_up_x,
    ...
    if choix_t:
        if not type_deplacement_tete:
            lecture, sens_lecture, direction_lecture, frame_lecture = deplacement_t(
                frame_lecture, lecture, direction_lecture, nombre_de_frame, sens_lecture,
                nb_frame_min_changement_lecture, probabilite_changement_selon_direction_t, liste_proba, indice_proba,
                compteur_de_frame)
    ...
    print(lecture, direction_lecture)
    ...
    img = img[posY - size_window_y // 2 + 1 - size_y % 2:posY + size_window_y // 2,
          posX - size_window_x // 2 + 1 - size_x % 2:posX + size_window_x // 2]
